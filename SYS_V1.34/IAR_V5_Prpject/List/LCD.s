...
Char_TAB_8x14:
        ...
        DC16 8208
//   15 {
//   16    0x0000,0x0000,0x0000,0x001C,0x0020,0x0040,0x0040,0x0040, // " 左上角
//   17    0x0040,0x0040,0x0020,0x001C,0x0000,0x0000,0x0000,0x0000, // # 右上角
//   18    0x0000,0x0000,0x0000,0xE000,0x1000,0x0800,0x0800,0x0800, // $ 左下角
//   19    0x0C30,0x0C60,0x00C0,0x0180,0x0300,0x0630,0x0C30,0x0000, // %    
//   20    0x0800,0x0800,0x1000,0xE000,0x0000,0x0000,0x0000,0x0000, // & 右下角
//   21    0x2010,0x2010,0x2010,0x2010,0x2010,0x2010,0x3FF0,0x0000, // ' 电池尾空
...
//   26    0x0000,0x0000,0x0000,0xFFFC,0x0000,0x0000,0x0000,0x0000, // , 竖线
...
//   41    0x0040,0x0040,0x0040,0x0040,0x0040,0x0040,0x0040,0x0040, // ; 上横线
...
//   73    0x0010,0x0030,0x0050,0x0190,0x0310,0x0C10,0x3FF0,0x0000, // [ 三角
//   74    0x0800,0x0800,0x0800,0x0800,0x0800,0x0800,0x0800,0x0800, // \ 下横线
//   75    0x0000,0x0000,0x0000,0xFC7C,0x0280,0x0280,0x0100,0x0100, // ] 指针
//   76    0x0200,0x0400,0x0C00,0x1FF0,0x3FF0,0x0C00,0x0400,0x0200, // ^ 上升沿
//   77    0x0100,0x0080,0x00C0,0x3FE0,0x3FF0,0x00C0,0x0080,0x0100, // _ 下降沿
//   78    0x2010,0x2010,0x2010,0x2010,0x2010,0x2010,0x2010,0x2010, // ` 电池身空
...
//  105    0x0780,0x0480,0x3CF0,0x2010,0x2FD0,0x2FD0,0x2FD0,0x2FD0, // { 电池头
//  106    0x2FD0,0x2FD0,0x2FD0,0x2FD0,0x2FD0,0x2FD0,0x2FD0,0x2FD0, // | 电池身
//  107    0x2FD0,0x2FD0,0x2FD0,0x2FD0,0x2FD0,0x2010,0x3FF0,0x0000, // } 电池尾
//  108    0x0780,0x0480,0x3CF0,0x2010,0x2010,0x2010,0x2010,0x2010 // ~ 空电池头
//  109 };
//  110 
//  111 /*******************************************************************************
//  112  LCD_Initial: 400x240 LCD显示模块初始化。 
//  113 *******************************************************************************/
...
//  251 /*******************************************************************************
//  252  LCD_WR_REG: 设置LCD模块上指定寄存器的值。 输入: 寄存器地址、寄存器值。
//  253 *******************************************************************************/
...
//  314 
//  315 /*******************************************************************************
//  316  delayms: 毫秒（mS）延时程序。 输入: 延时等待的毫秒数值（在72MHz主频情况下）
//  317 *******************************************************************************/
...
//  323 /*******************************************************************************
//  324  Point_SCR : 指定显示位置 for R61509V LCD   输入: X、Y坐标  
//  325 *******************************************************************************/
...
//  359 }
//  360 /*******************************************************************************
//  361  Display_Str: 在指定位置显示字符串   输入: X、Y坐标，颜色值，显示模式，字符串 
//  362 *******************************************************************************/
...
//  392       }
//  393     }
//  394     if(*s==0x21) x0 +=4;           //显示位置水平方向+4
//  395     else  x0 += 8;                 //显示位置水平方向+8
??Display_Str_4:
        ADDS     R5,R5,#+8
//  396     ++s;                           //字符串指针+1
??Display_Str_5:
...
//  399 /*******************************************************************************
//  400  Display_Logo: 在指定位置显示商标图案   输入: X、Y坐标 
//  401 *******************************************************************************/
...
//  410   y = y0;
        MOVS     R4,R1
//  411   for(i=0; i<16; i++){        // 0x36~0x75: 调色板(每色4B,顺序为B,G,R,Alpha)
        ADD      R1,SP,#+0
...
??Display_Logo_2:
        ...
        MOV      R0,R9
        BL       Point_SCR
//  417     LCD_PORT = Color[(Ptr[0x76 + i]&0xF0)>>4]; // 高半字节   
        LDRB     R0,[R5, #+0]
        ASRS     R0,R0,#+4
        LDRH     R0,[R8, R0, LSL #+1]
        STRH     R0,[R7, #+0]
...
??Display_Logo_3:
        ...
        MOV      R0,R9
        BL       Point_SCR
//  421     LCD_PORT = Color[Ptr[0x76 + i]&0x0F];      // 低半字节
        LDRB     R0,[R5, #+0]
        AND      R0,R0,#0xF
        LDRH     R0,[R8, R0, LSL #+1]
        STRH     R0,[R7, #+0]
